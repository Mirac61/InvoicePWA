  ...
  <section class="flex flex-col gap-4">
    ...
    <div class="flex items-center gap-3 flex-wrap">
      <div
        ...
        <svg width="15" height="15" viewBox="0 0 15 15" fill="none" class="shrink-0">
          ...
          <path d="M10 10l3.5 3.5" stroke="#999" stroke-width="1.4" stroke-linecap="round" />
        </svg>
      ...
      <div class="flex gap-1.5 flex-wrap">
        <button v-for="s in statuses" :key="s.val"
          class="px-4 py-2 rounded-full text-sm border transition-all duration-150" :class="filter === s.val
    ...
    <div class="bg-white border border-gray-200 rounded-xl overflow-hidden">
      <div v-if="loading" class="flex justify-center py-14">
        <div class="w-6 h-6 border-2 border-gray-100 border-t-blue-600 rounded-full animate-spin"></div>
      </div>
      <div v-else-if="filtered.length === 0" class="text-center py-16 text-gray-300 text-sm">
        Keine Rechnungen gefunden
      </div>
      <table v-else class="w-full border-collapse text-sm">
        <thead>
          <tr class="border-b border-gray-100">
            <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wide text-gray-400">
              Nummer
            </th>
            <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wide text-gray-400">
              Kunde
            ...
            <th class="px-4 py-3 text-right text-xs font-bold uppercase tracking-wide text-gray-400">
              Netto
            </th>
            <th class="px-4 py-3 text-right text-xs font-bold uppercase tracking-wide text-gray-400">
              Brutto
          ...
          <tr v-for="inv in filtered" :key="inv.id"
            ...
            <td class="px-4 py-3.5">
              <div class="flex items-center gap-2.5">
                ...
                <span class="text-gray-800">{{ inv.customer?.forename }}
            ...
            <td class="px-4 py-3.5 text-right tabular-nums font-semibold text-gray-900">
              {{ fmtMoney(inv.grossTotal) }}
            </td>
            <td class="px-4 py-3.5">
              <span class="inline-block px-2.5 py-1 rounded-full text-xs font-semibold whitespace-nowrap"
            ...
            <td class="px-4 py-3.5">
              <div class="flex gap-1.5 justify-end flex-wrap">
                ...
                <button class="act" @click="emit('edit', inv)">
                  <svg width="14" height="14" viewBox="0 0 16 16" fill="none">
                    <path d="M10.5 3l2.5 2.5L5 13.5H2.5V11L10.5 3z" stroke="currentColor" stroke-width="1.5"
                ...
                <button class="act" @click="emit('print', inv)">
                  <svg width="14" height="14" viewBox="0 0 16 16" fill="none">
                    <rect x="3" y="1.5" width="10" height="5" rx="1" stroke="currentColor" stroke-width="1.5" />
                    <path d="M3 6.5H1.5v7H3M13 6.5h1.5v7H13" stroke="currentColor" stroke-width="1.5" />
                    <rect x="3" y="9.5" width="10" height="5.5" rx="1" stroke="currentColor" stroke-width="1.5" />
                  </svg>
                  Drucken
                </button>
                <!-- Löschen nur bei DRAFT (GoBD) -->
                <button v-if="inv.status === 'DRAFT'" class="act act-del min-w-25" @click="emit('delete', inv)">
                  <svg width="14" height="14" viewBox="0 0 16 16" fill="none">
                    <path d="M2 4.5h12M5.5 4.5V3h5v1.5M6 7.5v5.5M10 7.5v5.5" stroke="currentColor" stroke-width="1.5"
                      stroke-linecap="round" />
                    <path d="M3 4.5l.5 10h9l.5-10" stroke="currentColor" stroke-width="1.5" stroke-linejoin="round" />
                  </svg>
                  Löschen
                </button>
                <!-- Stornieren bei SENT / PAID (GoBD) -->
                <button v-if="inv.status === 'SENT' || inv.status === 'PAID'" class="act act-del min-w-25"
